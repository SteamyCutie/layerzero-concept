...
contract MasterNetwork is Ownable, ILayerZeroReceiver, ILayerZeroUserApplicationConfig, IPOCDeployment {
    ...
    mapping(uint16 => uint) public counters;

    constructor(address _endpoint) {
        endpoint = ILayerZeroEndpoint(_endpoint);
    }

    function updateCounter(uint16 _chainId, bytes memory _dstAddress, uint _amount, string memory _method) external payable override {
        uint amount;
        bytes memory method = bytes(_method);
        bytes memory expectAdd = bytes("ADD");
        bytes memory expectSub = bytes("SUB");
        if(method.length == expectAdd.length && keccak256(method) == keccak256(expectAdd))
            amount = counters[_chainId].add(_amount);
        else if(method.length == expectSub.length && keccak256(method) == keccak256(expectSub))
            amount = counters[_chainId].sub(_amount);
        else
            amount = counters[_chainId].mul(_amount);

        endpoint.send{value: msg.value}(_chainId, _dstAddress, bytes(""), payable(msg.sender), address(0x0), abi.encodePacked(amount));
    }

    function requestCounter(uint16 _chainId, bytes memory _dstAddress) external payable override {
    ...
    function lzReceive(
        ...
        counters[_srcChainId] = bytesToUint(_payload);
    }
